...
class CustomUserJsonSerializer implements JsonSerializer<CustomUser> {
  /// Default configuration for this serializer.
  final SerializationConfig _defaultConfig;

  /// Creates a serializer with the specified default configuration.
  CustomUserJsonSerializer([SerializationConfig? defaultConfig])
  ...
  @override
  CustomUser fromJson(dynamic json, [SerializationConfig? config]) {
    final effectiveConfig = config ?? _defaultConfig;
    if (json == null) {
      throw DeserializationException(
        'Cannot deserialize CustomUser from null JSON',
  ...
  static Map<String, dynamic> encode(
    ...
    return CustomUserJsonSerializer().toJson(instance, config);
  }

  /// Convenience method for static access with default configuration
  static CustomUser decode(dynamic json, [SerializationConfig? config]) {
    return CustomUserJsonSerializer().fromJson(json, config);
  }
}
...
/// Generated abstract base SQLite repository for [CustomUser] aggregate.
///
/// This abstract class implements the base Repository<CustomUser> methods
/// and declares custom methods as abstract. Extend this class and
/// implement the abstract methods.
abstract class CustomUserSqliteRepositoryBase implements CustomUserRepository {
  /// Creates a repository instance.
  ///
  /// [connection] - An open SQLite database connection.
  CustomUserSqliteRepositoryBase(this._connection);

  /// The SQLite database connection (protected).
  final SqliteConnection _connection;

  /// The SQL dialect for SQLite (protected).
  final _dialect = SqliteDialect();

  /// The JSON serializer (protected).
  final _serializer = CustomUserJsonSerializer();

  /// The table name for CustomUser aggregates.
    ...
    return await _connection.transaction(() async {
      try {
        ...
        final rows = await _connection.query(
          'SELECT * FROM "custom_users" WHERE "id" = ?',
  ...
  Future<void> save(CustomUser aggregate) async {
    ...
    await _connection.transaction(() async {
      try {
        ...
        await _connection.execute(
          'INSERT OR REPLACE INTO "custom_users" (${columns.map((c) => '"$c"').join(', ')}) VALUES ($placeholders)',
          values,
        );
      } catch (e) {
  ...
  Future<void> deleteById(UuidValue id) async {
    await _connection.transaction(() async {
      try {
        ...
        final rows = await _connection.query(
          'SELECT "id" FROM "custom_users" WHERE "id" = ?',
          [_dialect.encodeUuid(id)],
        );
        ...
        await _connection.execute('DELETE FROM "custom_users" WHERE "id" = ?', [
          _dialect.encodeUuid(id),
        ]);
      } on RepositoryException {
        rethrow;
