...
@Serializable()
@GenerateSqliteRepository(implements: ${conditionalInterface ? 'ConditionalStore' : 'null'})
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final repository = TestUserSqliteRepository(connection);
      await expectLater(
...
class _UnusedConnection implements SqliteConnection {
  int calls = 0;
  @override
...
class _VersionedSubtype extends TestUser implements VersionedAggregateRoot {
  _VersionedSubtype() : super(name: 'test', email: 'test', isActive: true);
  @override
