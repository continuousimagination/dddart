...
void main() {
  group('Optional Logging Behavior - HTTP', () {
    ...
    test('CrudResource works without logging configuration', () async {
      ...
      final getResponse = await resource.handleGetById(getRequest, product.id.toString());
    ...
    test('CrudResource handles PUT without logging configuration', () async {
      ...
      final product = TestProduct(name: 'Original', price: 10.0);
      await repository.save(product);
      ...
      final putResponse = await resource.handleUpdate(putRequest, product.id.toString());
    ...
    test('CrudResource handles DELETE without logging configuration', () async {
      ...
      // Create product
      final product = TestProduct(name: 'To Delete', price: 5.0);
      await repository.save(product);
      ...
      final deleteResponse = await resource.handleDelete(deleteRequest, product.id.toString());
    ...
    test('CrudResource handles errors without logging configuration', () async {
      ...
      final response = await resource.handleGetById(getRequest, nonExistentId.toString());
    ...
    test('Multiple HTTP operations work without logging', () async {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        final response = await resource.handleGetById(getRequest, product.id.toString());
    ...
    test('CrudResource works when Logger.root.level is OFF', () async {
      ...
      final product = TestProduct(name: 'Test', price: 100.0);
      await repository.save(product);
      ...
      final response = await resource.handleGetById(request, product.id.toString());
      expect(response.statusCode, equals(200));
    });
