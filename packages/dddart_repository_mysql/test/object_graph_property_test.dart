...
void main() {
  group('Object Graph Property Tests', () {
    ...
    group('Property 2: Object graph persistence round-trip', () {
            ...
            for (var i = 0; i < 100; i++) {
              ...
              for (var j = 0; j < order.items.length; j++) {
                ...
                expect(
                  ...
                  reason:
                      'Iteration $i, Item $j: Product name should be '
                      'preserved',
                );
                expect(
                  retrievedItem.quantity,
                  equals(originalItem.quantity),
                  reason: 'Iteration $i, Item $j: Quantity should be preserved',
                );
                expect(
                  retrievedItem.unitPrice,
                  equals(originalItem.unitPrice),
                  reason:
                      'Iteration $i, Item $j: Unit price should be '
                      'preserved',
                );
              }
      ...
      test('should preserve aggregate with value objects when saving then '
          'loading', () async {
        final helper = createTestHelper();
        await helper.connect();
          ...
          for (var i = 0; i < 100; i++) {
            ...
            if (customer.billingAddress == null) {
              ...
              expect(
                ...
                reason: 'Iteration $i: Billing zip code should be preserved',
              );
            }
      ...
      test('should handle empty entity collections in round-trip', () async {
        final helper = createTestHelper();
        await helper.connect();
      ...
      test('should handle updates to existing aggregates', () async {
        final helper = createTestHelper();
        await helper.connect();
        ...
        try {
          ...
          for (var i = 0; i < 50; i++) {
            ...
            expect(
              ...
              reason:
                  'Iteration $i: Updated customer name should be '
                  'preserved',
            );
            expect(
              retrieved.items.length,
              equals(updatedOrder.items.length),
              reason: 'Iteration $i: Updated items count should be preserved',
            );
...
Address _generateRandomAddress(Random random) {
  return Address(
    street:
        '${random.nextInt(9999) + 1} ${_generateRandomString(random, 10)} '
