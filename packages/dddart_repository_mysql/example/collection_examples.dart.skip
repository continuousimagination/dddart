...
Future<void> primitiveCollectionsExample(MysqlConnection connection) async {
  print('\n=== Primitive Collections Example ===\n');

  final repository = UserMysqlRepository(connection);
...
Future<void> valueObjectCollectionsExample(MysqlConnection connection) async {
...
Future<void> entityCollectionsExample(MysqlConnection connection) async {
  print('\n=== Entity Collections Example ===\n');

  final repository = ShoppingCartMysqlRepository(connection);
  await repository.createTables();
...
@Serializable()
@GenerateMysqlRepository(tableName: 'products')
...
Future<void> nullableCollectionsExample(MysqlConnection connection) async {
  print('\n=== Nullable Collections Example ===\n');

  final repository = ProductMysqlRepository(connection);
  await repository.createTables();
...
Future<void> cascadeDeleteExample(MysqlConnection connection) async {
...
void main() async {
  print('DDDart MySQL Collection Examples');
  print('=================================');
  ...
  final database = Platform.environment['MYSQL_DATABASE'] ?? 'dddart_examples';
  ...
  try {
    ...
    await primitiveCollectionsExample(connection);
    await valueObjectCollectionsExample(connection);
    await entityCollectionsExample(connection);
    await nullableCollectionsExample(connection);
    await cascadeDeleteExample(connection);
