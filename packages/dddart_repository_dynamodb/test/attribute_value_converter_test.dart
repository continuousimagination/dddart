  ...
  group('AttributeValueConverter', () {
    ...
    group('Property 4: AttributeValue conversion round-trip', () {
      ...
      test('should round-trip null values', () {
        for (var i = 0; i < 100; i++) {
          const original = null;
          final attributeValue = AttributeValueConverter.jsonToAttributeValue(
            original,
          );
          final roundTripped = AttributeValueConverter.attributeValueToJson(
            attributeValue,
          );
      ...
      test('should round-trip complex JSON objects', () {
        for (var i = 0; i < 100; i++) {
          final original = _generateRandomMap(random, depth: 0);
          final attributeMap = AttributeValueConverter.jsonMapToAttributeMap(
            original,
          );
          final roundTripped = AttributeValueConverter.attributeMapToJsonMap(
            attributeMap,
          );
          expect(roundTripped, equals(original), reason: 'Iteration $i failed');
        }
